...
import { startInterview, submitAnswer, getNextQuestion } from "../services/interview.service.js";
...
const STATES = {
  ...
  GENERATING: "generating", // waiting for next question from AI
...
export default function Interview() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      } else if (data.generatingQuestion) {
        // Next question is being generated by AI in background — poll for it
        setQIndex(data.questionNumber || qIndex + 1);
        if (data.totalTopics) setTotalTopics(data.totalTopics);
        setUiState(STATES.GENERATING);
        pollForNextQuestion(sessionId);
  ...
  // Poll for next question while AI generates it
  const pollForNextQuestion = async (sid) => {
    const POLL_MS = 3000;
    const MAX_POLLS = 60; // 60 * 3s = 3 min max wait (14b model can take 60-90s)
  ...
  const inInterview = uiState === STATES.ACTIVE || uiState === STATES.SUBMITTING || uiState === STATES.GENERATING;
        ...
        <div className="interview-main">
          ...
          {uiState === STATES.PREVIEW && (
            <div className="interview-start card">
              ...
              >
                {permGranted ? "Everything Looks Good — Continue →" : "Continue (Simulation Mode) →"}
              </button>
            </div>
          )}
              ...
              <div className="question-card card">
                <div className="q-meta" style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
                    🔄 Adaptive interview · min 5 questions
                  </span>
                  {totalTopics > 0 && (
                    <span style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
                ...
                {uiState === STATES.GENERATING ? (
                  <div style={{ textAlign: "center", padding: "2rem 0" }}>
                    <div className="spinner" style={{ margin: "0 auto" }} />
                    <p style={{ marginTop: "1rem", color: "var(--text-muted)", fontSize: "0.9rem" }}>
                      🧠 AI is preparing your next question based on your answer...
                    </p>
                  </div>
                ) : (
              ...
              <div className="answer-area">
                <label className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  <textarea
                    ...
                    disabled={uiState === STATES.SUBMITTING || uiState === STATES.GENERATING}
                  />
                </label>
                ...
                <button
                  ...
                  disabled={uiState === STATES.SUBMITTING || uiState === STATES.GENERATING || !answer.trim()}
                >
                  {uiState === STATES.SUBMITTING ? (
                    <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
                      <span className="spinner-sm" />
                      {qIndex >= 5 ? "Checking topic coverage…" : "Generating next question…"}
                    </span>
                  ) : "Submit Answer →"}
                </button>
              </div>
